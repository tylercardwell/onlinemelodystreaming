...
class YoutubeAudioSearch
{
    public function search(
        ...
        if (settings('youtube.search_method') === 'site') {
    ...
    private function viaApi(string $artistName, string $trackName)
    {
        $response = Http::throw()
            ...
            ->get(
                ...
                $this->getParams($artistName, $trackName),
            );
    ...
    private function getParams($artist, $track)
    {
        $params = [
            ...
            'key' => settings()->getRandom('youtube_api_key'),
            'part' => 'snippet',
        ...
        if ($regionCode = settings('youtube.region_code')) {
