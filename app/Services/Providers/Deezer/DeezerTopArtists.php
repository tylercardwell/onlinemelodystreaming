...
class DeezerTopArtists extends DeezerBase implements ContentProvider
{
    public function getContent(): Collection
    {
        ...
        $response =
            $this->getResponseData(
                Http::timeout(5)->get(
                    "{$this->baseUrl}/editorial/0/charts?limit=50&country=US",
                ),
            )['artists']['data'] ?? [];

        $normalizedArtists = collect($response)->map(
            fn($artist) => $this->normalizer->artist($artist),
        );
        $originalOrder = $normalizedArtists->keyBy('name');

        $savedData = (new StoreExternalDataInDb('deezer_id'))->execute(
            artists: $normalizedArtists,
        );

        return collect($savedData['artists'])
