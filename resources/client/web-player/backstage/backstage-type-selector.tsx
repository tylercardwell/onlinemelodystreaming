...
export function Component() {
  ...
  return (
    <BackstageLayout>
      <div className="my:20 mx-auto max-w-195 md:my-10">
        <h1 className="mb-2.5 text-center text-3xl font-medium md:text-5xl">
          <Trans
            message="Get access to :siteName for artists"
            values={{siteName: branding.site_name}}
          />
        </h1>
        <h2 className="mb-13.5 text-center text-lg font-medium">
          <Trans message="First, select the type of your request" />
        </h2>
        <div className="items-center gap-13.5 md:flex">
          <ClaimPanelLayout
...
function ClaimPanelLayout({
  ...
  return (
    <Link
      to={link}
      className={clsx(
        'rounded-card hover:bg-primary/4 bg-card flex flex-auto cursor-pointer flex-col items-center justify-center border-2 p-8.5 transition-shadow hover:shadow-xl',
        className,
      )}
    >
      <h3 className="mb-2.5 text-lg font-medium">{title}</h3>
      <img className="h-33 w-33 rounded-full object-cover" src={image} alt="" />
    </Link>
  );
}
