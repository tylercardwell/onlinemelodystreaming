    ...
    <>
      ...
      <div className="relative">
        <CommentBarContextProvider>
          <AdHost slot="general_top" className="mb-11" />
        ...
        />
        <AdHost slot="album_above" className="mt-8.5" />
        ...
        <AdHost slot="general_bottom" className="mt-11" />
      </div>
    </>
  );
}
...
function AlbumPageHeader({album}: PlaylistPageHeaderProps) {
  ...
  return (
    <Fragment>
      <MediaPageHeaderLayout
        centerItems
        image={<AlbumImage album={album} className="rounded-card" />}
        title={album.name}
        subtitle={
          <>
            {!!album.genres?.length ? (
              <GenreList genres={album.genres} className="mb-6" />
            ) : null}
            <AvatarGroup>
        ...
        description={
          <BulletSeparatedItems className="text-muted-foreground text-sm">
            {album.tracks?.length ? (
              <Trans
