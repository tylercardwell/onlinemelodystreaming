...
import {cloneElement, CSSProperties, FormEvent, ReactElement, ReactNode, useState} from 'react';
import {Link, useNavigate} from 'react-router';

const optionOneAssets = {
  hero: new URL('../../images/landing-option-one/hero-collage.png', import.meta.url).href,
  heroCovers: [
    new URL('../../images/landing-option-one/hero-cover-terrain-hd.png', import.meta.url).href,
    new URL('../../images/landing-option-one/hero-cover-moon-hd.png', import.meta.url).href,
    new URL('../../images/landing-option-one/hero-cover-flowers-hd.png', import.meta.url).href,
    new URL('../../images/landing-option-one/hero-cover-city-hd.png', import.meta.url).href,
    new URL('../../images/landing-option-one/hero-cover-vinyl-hd.png', import.meta.url).href,
  ],
  editorial: new URL('../../images/landing-option-one/editorial-discovery-backdrop.png', import.meta.url).href,
  mountainCatalog: new URL('../../images/landing-option-one/mountain-catalog-backdrop.png', import.meta.url).href,
  ...
  benefits: [
    new URL('../../images/landing-option-one/benefit-catalog-hd.png', import.meta.url).href,
    new URL('../../images/landing-option-one/benefit-fresh-hd.png', import.meta.url).href,
    new URL('../../images/landing-option-one/benefit-live-hd.png', import.meta.url).href,
  ],
};
...
export function DiscoveryHeroSection({config}: {config: DiscoveryHeroConfig}) {
  ...
  return <section className="relative isolate overflow-hidden bg-[#080a09] text-white"><Navbar.Root className="relative z-30 mx-auto max-w-[1536px] border-b border-white/10 bg-[#080a09] text-white dark:bg-[#080a09]"><Navbar.Logo color="light" url="/" /><nav className="ml-10 hidden items-center gap-7 text-xs font-bold tracking-[0.12em] text-white/55 uppercase md:flex"><Link to="/" className="text-white transition duration-200 hover:scale-[1.04] hover:text-[#1ed760]">Home</Link><Link to="/discover" className="transition duration-200 hover:scale-[1.04] hover:text-[#1ed760]">Discover</Link><Link to="/search" className="transition duration-200 hover:scale-[1.04] hover:text-[#1ed760]">Browse</Link></nav><Navbar.Content className="ml-auto"><Navbar.AuthContent /></Navbar.Content></Navbar.Root><div className="pointer-events-none absolute top-[-24rem] left-[48%] -z-10 size-[58rem] rounded-full bg-[#1ed760]/12 blur-[150px]" /><div className="mx-auto grid max-w-[1536px] items-center gap-7 px-6 pt-10 pb-10 sm:px-10 sm:pt-12 sm:pb-12 lg:grid-cols-[.92fr_1.08fr] lg:gap-10 lg:px-16 lg:pt-14 lg:pb-14"><div className="relative z-10 max-w-2xl"><h1 className="mt-5 text-5xl font-black tracking-[-0.07em] text-balance sm:text-7xl lg:text-[5.8rem] lg:leading-[.91]">{config.title}</h1><p className="mt-5 max-w-xl text-lg leading-7 text-white/65 sm:text-xl">{config.description}</p><form onSubmit={submitSearch} className="mt-7 max-w-xl"><label className="sr-only" htmlFor="discovery-search">Search music</label><div className="flex h-13 items-center rounded-full border border-white/15 bg-white/8 px-5 transition focus-within:border-[#1ed760] focus-within:bg-white/12"><SearchIcon className="size-5 shrink-0 text-white/55" /><input id="discovery-search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search artists, albums, songs..." className="h-full min-w-0 flex-1 bg-transparent px-3 text-sm text-white outline-none placeholder:text-white/42" /><button type="submit" className="text-xs font-bold tracking-[0.12em] text-[#1ed760] uppercase transition hover:scale-[1.04] hover:text-white">Search</button></div></form><Buttons buttons={config.buttons ?? []} className="mt-8 flex flex-wrap gap-3 [&_a]:transition-transform [&_a:hover]:scale-[1.03]" /><div className="mt-7 flex flex-wrap gap-x-7 gap-y-3 text-xs font-bold tracking-[0.13em] text-white/45 uppercase"><span>New releases</span><span>Indie</span><span>Electronic</span><span>Hip hop</span><span>More</span></div></div>{config.referenceAssets ? <HeroArtGrid /> : <CoverField covers={covers} />}</div></section>;
}
...
const heroArtTitles = ['Contours after dark', 'Blue hour', 'Quiet bloom', 'City in motion', 'Side A'];
function HeroArtGrid() {
  return <div className="hero-cover-wall mx-auto w-full max-w-2xl" aria-label="Featured music">{optionOneAssets.heroCovers.map((src, index) => <Link key={src} to="/discover" aria-label={`Explore ${heroArtTitles[index]}`} className="hero-art-tile group" style={{'--hero-art-delay': `${index * -0.8}s`, '--hero-art-rotate': `${[-8, 6, -3, 9, -11][index]}deg`} as CSSProperties}><span className="hero-art-frame"><img src={src} alt={heroArtTitles[index]} className="aspect-square size-full object-cover" /><span className="hero-art-glint" aria-hidden="true" /><span className="hero-art-caption">Explore</span></span></Link>)}</div>
}
...
export function DiscoveryBenefitsSection({config}: {config: DiscoveryBenefitsConfig}) {
  return <section className="bg-[#080a09] px-6 py-12 text-white sm:px-10 lg:px-16 lg:py-16"><div className="mx-auto max-w-[1536px]"><p className="text-xs font-bold tracking-[0.2em] text-[#1ed760] uppercase">{config.eyebrow}</p><h2 className="mt-3 text-4xl font-black tracking-[-0.06em] sm:text-5xl">{config.title}</h2><p className="mt-3 max-w-xl text-white/60">{config.description}</p><div className="mt-8 grid gap-4 md:grid-cols-3">{config.benefits?.map((item, index) => <article key={item.title} className="group relative min-h-72 overflow-hidden rounded-lg border border-white/10 bg-[#121614] p-5 transition duration-200 ease-out hover:z-10 hover:scale-[1.025] hover:border-white/20 hover:shadow-2xl hover:shadow-black/40 focus-within:z-10 focus-within:scale-[1.025] motion-reduce:transform-none"><img src={config.referenceAssets ? optionOneAssets.benefits[index] : item.image?.src} alt="" className="absolute inset-0 size-full object-cover opacity-65 transition duration-300 group-hover:scale-105 motion-reduce:transform-none" /><div className="absolute inset-0 bg-linear-to-t from-[#121614] via-[#121614]/45 to-transparent" /><div className="relative flex h-full flex-col justify-end"><h3 className="text-xl font-black tracking-[-0.05em]">{item.title}</h3><p className="mt-3 max-w-xs text-sm leading-6 text-white/70">{item.description}</p></div></article>)}</div></div></section>;
}
...
export function CatalogCtaSection({config}: {config: CatalogCtaConfig}) {
  ...
  const backdropImage = config.referenceAssets
    ? optionOneAssets.mountainCatalog
    : config.image?.src;

  return (
    <section className="bg-[#080a09] px-6 py-12 text-white sm:px-10 lg:px-16 lg:py-16">
      <div className="relative mx-auto max-w-[1536px] overflow-hidden rounded-2xl border border-white/10 bg-[#07120f] px-7 py-12 shadow-[0_24px_80px_rgba(0,0,0,.35)] sm:px-12 sm:py-14">
        {backdropImage ? (
          <img src={backdropImage} alt="" aria-hidden="true" className="absolute inset-0 size-full object-cover object-center opacity-70" />
        ) : null}
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_14%_8%,rgba(74,184,136,.16),transparent_25%),linear-gradient(90deg,rgba(3,12,11,.78)_0%,rgba(3,15,15,.58)_42%,rgba(3,13,16,.4)_100%)]" />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[44%] bg-[linear-gradient(155deg,transparent_0%,transparent_25%,rgba(6,25,27,.72)_26%,rgba(3,13,16,.96)_70%)]" />
        <div className="pointer-events-none absolute inset-x-0 bottom-[19%] h-px bg-white/10" />
        <div className="relative grid items-end gap-9 lg:grid-cols-[.88fr_1.12fr] lg:gap-16">
          <div>
            <h2 className="mt-3 max-w-xl text-4xl font-black tracking-[-0.065em] text-balance sm:text-5xl">{config.title}</h2>
            {config.description ? <p className="mt-4 max-w-lg text-base leading-7 text-white/72">{config.description}</p> : null}
          </div>
          <form onSubmit={submitSearch} className="w-full" role="search">
            ...
            <div className="flex h-14 items-center rounded-full border border-white/25 bg-white/10 p-1.5 pl-5 shadow-[inset_0_1px_0_rgba(255,255,255,.12)] backdrop-blur-md transition duration-200 focus-within:border-[#55dd91] focus-within:bg-white/14 focus-within:shadow-[0_0_0_4px_rgba(85,221,145,.13)]">
              <SearchIcon className="size-5 shrink-0 text-white/70" aria-hidden="true" />
              <input id="catalog-search" type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search artists, albums, songs..." className="h-full min-w-0 flex-1 bg-transparent px-3 text-sm text-white outline-none placeholder:text-white/55" />
              <button type="submit" className="h-full shrink-0 rounded-full bg-[#55dd91] px-5 text-xs font-extrabold tracking-[0.1em] text-[#062116] uppercase transition duration-200 hover:scale-[1.03] hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#55dd91] active:scale-[.98]">Search</button>
            </div>
          </form>
        </div>
      </div>
