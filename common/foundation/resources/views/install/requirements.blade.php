<x-install-layout>
    @if($allPassed)
        <p class="mb-6">Your server meets all the requirements for {{config('app.name')}}. Click continue button below to proceed with the installation.</p>
    @else
        <p class="mb-6">We've found some issues that need to be fixed before you can proceed with the installation.</p>
    @endif
        ...
        <div class="border-b py-3">
            ...
            @if(!$group['allPassed'])
                @foreach($group['items'] as $item)
                    @if(!$item['passes'])
                        <p class="text-sm text-destructive-foreground mt-2">{!! $item['errorMessage'] !!}</p>
                    @endif
                @endforeach
            @endif
        </div>
