...
export function SliderThumb({
  ...
  const className = clsx(
    'absolute inset-0 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full outline-hidden transition-button duration-200',
    thumbSize,
  ...
  return (
    ...
    >
      <input
        id={thumbIds[index]}
        onKeyDown={ignoreEventsFromPortal(() => {
          updateDraggedThumbs(index, true);
        })}
        onKeyUp={ignoreEventsFromPortal(() => {
...
function thumbColor({isDisabled, fillColor}: SliderThumbColorProps): string {
  if (isDisabled) {
  ...
  return clsx('bg-primary hover:ring hover:ring-primary');
}
