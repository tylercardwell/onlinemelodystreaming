...
export function ChipField<T>({ref, ...props}: ChipFieldProps<T>) {
  ...
  const loadingIndicator = (
    <ProgressCircle isIndeterminate size="w-6 h-6" aria-label="loading..." />
  );
...
function ChipInput<T>(props: ChipInputProps<T>) {
  ...
  const handleFocusAndClick = ignoreEventsFromPortal(() => {
  ...
  return (
    ...
    >
      <input
        ...
        type="text"
        className={clsx(
          'mx-2 my-1 min-w-7.5 flex-[1_1_60px] bg-transparent text-sm outline-hidden',
          size === 'xs' ? 'h-5' : 'h-7.5',
        )}
        placeholder={placeholder}
