...
export const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  (props, ref) => {
    ...
    const mergedClassName = clsx(
      ...
      !invalid && disabled && 'text-foreground/30',
    );

    let CheckboxIcon: ComponentType<SvgIconProps>;
    let checkboxColor = invalid ? 'text-destructive' : null;
    if (isIndeterminate) {
      ...
      CheckboxIcon = Icon;
      checkboxColor = checkboxColor || 'text-muted-foreground';
    }
    ...
    return (
      <div>
        <label className={mergedClassName}>
          ...
          >
            <input
              className="absolute top-0 left-0 h-6 w-6 appearance-none overflow-hidden rounded-sm outline-hidden transition-shadow ring-inset focus-visible:ring"
              type="checkbox"
            ...
            <CheckboxIcon
              ...
              className={clsx(
                'pointer-events-none',
                disabled ? 'text-foreground/30' : checkboxColor,
              )}
            />
            {children && (
              <div
                className={clsx(
                  ...
                  style.size.font,
                  orientation === 'vertical' ? 'mb-1.5' : 'ml-1.5',
                )}
              >
                {children}
              </div>
