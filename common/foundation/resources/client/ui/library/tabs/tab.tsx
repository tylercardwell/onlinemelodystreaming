...
export function Tab({
  ...
  const padding = paddingProp || (size === 'sm' ? 'px-3' : 'px-4.5');

  const mergedClassname = clsx(
    'tracking-wide overflow-hidden text-sm flex items-center justify-center outline-hidden transition-colors',
    'focus-visible:ring focus-visible:ring-2 ring-inset rounded-sm whitespace-nowrap cursor-pointer',
    width,
    textColor({isDisabled, isSelected}),
    className,
    size === 'md' && `${padding} ${height || 'h-12'}`,
    size === 'sm' && `${padding} ${height || 'h-8'}`,
...
function textColor({isDisabled, isSelected}: TextColorProps): string {
  if (isDisabled) {
    return 'text-foreground/30 cursor-default';
  }
  if (isSelected) {
    return 'text-primary';
  }
