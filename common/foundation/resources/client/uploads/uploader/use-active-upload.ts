import {deleteFileEntriesOptions} from '@common/admin/file-entry/file-entries-queries';
...
export function useActiveUpload() {
  const deleteFileEntries = useMutation(deleteFileEntriesOptions);
  ...
  const deleteEntry = useCallback(
    ({onSuccess, entryPath}: DeleteEntryProps) => {
      ...
      deleteFileEntries.mutate(
        ...
        {onSuccess: handleSuccess, onError: err => showHttpErrorToast(err)},
      );
    },
