...
class BuildGoogleAnalyticsReport extends BuildAnalyticsReport
{
    ...
    protected function buildReport(): array
    {
        ...
        if ($this->compareDateRange) {
            $previous = $this->buildTopBrowsersMetric($this->compareDateRange);
            foreach ($browsers['data'] as $key => $item) {
                if (isset($previous[$key])) {
                    $browsers['data'][$key]['previousValue'] =
                        $previous[$key]['value'];
                }
            }
        }
        ...
        if ($this->compareDateRange) {
            $previous = $this->buildTopLocationsMetric($this->compareDateRange);
            foreach ($locations['data'] as $key => $item) {
                if (isset($previous[$key])) {
                    $locations['data'][$key]['previousValue'] =
                        $previous[$key]['value'];
                }
            }
        }
        ...
        if ($this->compareDateRange) {
            $previous = $this->buildTopDevicesMetric($this->compareDateRange);
            foreach ($devices['data'] as $key => $item) {
                if (isset($previous[$key])) {
                    $devices['data'][$key]['previousValue'] =
                        $previous[$key]['value'];
                }
            }
        }
        ...
        if ($this->compareDateRange) {
            $previous = $this->buildTopPlatformsMetric($this->compareDateRange);
            foreach ($platforms['data'] as $key => $item) {
                if (isset($previous[$key])) {
                    $platforms['data'][$key]['previousValue'] =
                        $previous[$key]['value'];
                }
            }
        }
        ...
        // page views
        $pageViewsData = $this->getPageViews($this->dateRange);
        $pageViews = [
            'granularity' => $this->dateRange->granularity,
            'total' => $pageViewsData->sum('value'),
            'data' => $pageViewsData,
        ];
        if ($this->compareDateRange) {
            $previous = $this->getPageViews($this->compareDateRange);
            foreach ($pageViews['data'] as $key => $item) {
                if (isset($previous[$key])) {
                    $pageViews['data'][$key]['previousValue'] =
                        $previous[$key]['value'];
                }
            }
        }
