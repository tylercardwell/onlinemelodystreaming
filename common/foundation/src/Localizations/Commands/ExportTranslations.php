...
class ExportTranslations extends Command
{
    ...
    protected function getLaravelTranslationMessages()
    {
        ...
        $lines = $files
            ->map(function (SplFileInfo $file) {
                ...
                if (
                    Str::contains($contents, 'extends FormRequest') &&
                    Str::contains($contents, 'function messages')
                ) {
                    $lines = array_merge(
                        $lines,
    ...
    {
        $lines = collect((new PermissionConfigLoader())->get())
            ->map(function (PermissionConfigItem $permission) {
                $restrictionLines = collect($permission->restrictions ?? [])
                    ->map(function (RestrictionConfigItem $restriction) {
                        return [
