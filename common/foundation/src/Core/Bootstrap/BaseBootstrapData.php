...
class BaseBootstrapData implements BootstrapData
{
    ...
    public array $cssThemes = [];
    public CssTheme $initialTheme;

    public function getEncoded(): string
    {
        if ($this->data['user']) {
            $this->data['user'] = new UserResource($this->data['user']);
        }
    ...
    protected function setCssThemes(
        string $defaultSchemeSetting = 'themes.default_scheme',
        ...
        $defaultDark =
            $themes->where('default_dark', true)->first() ?? $defaultLight;
