...
namespace Common\Settings\Themes;
...
class CssThemePolicy extends BasePolicy
{
    ...
    {
        return $user->hasPermission('cssTheme.view') ||
            $user->id === (int) $userId;
    }
    ...
    {
        return $user->hasPermission('cssTheme.update') ||
            $cssTheme->user_id === $user->id;
    }
    ...
    public function destroy(BaseUser $user, CssTheme $theme)
    {
        if (
            $theme->default_dark &&
            app(CssTheme::class)->where('default_dark', true)->count() < 2
        ) {
        ...
        if (
            $theme->default_light &&
            app(CssTheme::class)->where('default_light', true)->count() < 2
        ) {
        ...
        return $user->hasPermission('cssTheme.delete') ||
