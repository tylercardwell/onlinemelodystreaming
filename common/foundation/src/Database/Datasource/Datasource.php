...
class Datasource
{
    ...
    public function __construct(
        ...
        protected string $defaultOrderCol = 'updated_at',
    ...
    public function getOrder(): array
    {
        if (isset($this->order['col'])) {
            ...
            $specifiedOrder = $this->param('order') ?: $this->param('sort')
        ) {
            $parts = preg_split('(\||:)', $specifiedOrder);
            $orderCol = Arr::get($parts, 0, $this->defaultOrderCol);
            $orderDir = Arr::get($parts, 1, $this->defaultOrderDir);
        ...
        if (
            $orderCol !== 'relevance' &&
            !Str::endsWith($orderCol, '_count') &&
            $this->qualifySortColumns
        ) {
